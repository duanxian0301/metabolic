...
repo_dir <- "D:/codex/metabolic_repo/external_support_karjalainen233/inputs/external_bivariate_round2/nonlipid"
ascii_work_dir <- "D:/codex/GenomicSEM/metabolic/external_bivariate_nonlipid_round2_work"
out_dir <- "D:/metabolic/233/ldsc_univariate/external_bivariate_round2_nonlipid"
...
factor_meta <- data.table(
  trait = c("nonlipid8_F1", "nonlipid8_F2", "nonlipid8_F3"),
  source_file = c(
...
ldsc_out <- ldsc(
  ...
  ldsc.log = file.path(out_dir, "external_bivariate_nonlipid_round2")
)

saveRDS(ldsc_out, file.path(out_dir, "external_bivariate_nonlipid_round2.rds"))
...
fwrite(traits_manifest, file.path(out_dir, "external_bivariate_nonlipid_round2_traits_manifest.tsv"), sep = "\t")
fwrite(diag_dt, file.path(out_dir, "external_bivariate_nonlipid_round2_h2.tsv"), sep = "\t")
fwrite(as.data.table(R, keep.rownames = "trait"), file.path(out_dir, "external_bivariate_nonlipid_round2_rg_matrix.tsv"), sep = "\t")
fwrite(pairs_dt, file.path(out_dir, "external_bivariate_nonlipid_round2_requested_pairs.tsv"), sep = "\t")
fwrite(summary_dt, file.path(out_dir, "external_bivariate_nonlipid_round2_pairset_summary.tsv"), sep = "\t")
fwrite(contrast_dt, file.path(out_dir, "external_bivariate_nonlipid_round2_factor_contrast.tsv"), sep = "\t")

message("External nonlipid round2 bivariate LDSC written to: ", out_dir)
